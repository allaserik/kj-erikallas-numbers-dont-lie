...
- This is context-guided compliance, not a full deterministic post-filter rule engine.
- Limitation acknowledged: explicit hard-filtering can be added as next step.
...
- Transit: intended via HTTPS in deployment (local dev uses HTTP for convenience).
- At rest: not implemented at application layer in this submission; treated as deployment/infra hardening item.
- This is listed as known limitation with a production plan.
