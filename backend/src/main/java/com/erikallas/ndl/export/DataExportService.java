...
import com.erikallas.ndl.ai.insight.AiInsightRepository;
...
public class DataExportService {
    ...
    public DataExportResponse exportForUser(UUID userId) {
        ...
        var twoFactor = twoFactorSecretRepository.findByUserId(userId)
                .map(tf -> new TwoFactorData(tf.isEnabled(), tf.getVerifiedAt(), tf.getCreatedAt(), tf.getUpdatedAt()))
        ...
        var weights = weightEntryRepository.findByUserIdOrderByMeasuredAtDesc(userId).stream()
                .map(w -> new WeightEntryData(w.getId(), w.getMeasuredAt(), w.getWeightKg(), w.getNote()))
                .toList();
        ...
        return new DataExportResponse(
                OffsetDateTime.now(),
                new AccountData(user.getId(), user.getAuth0Sub(), user.getEmail(), user.getEmailVerified(),
